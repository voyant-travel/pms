...
/**
 * Common chrome for the Housekeeping pages: a titled header with the shared
 * property selector (reused from the ARI surface, so the selected property follows
 * the operator across Rates & Inventory, Front Desk and Housekeeping) plus optional
 ...
 * Mirrors `front-desk-page-shell.tsx`.
 */
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      {!isLoading && options && options.length === 0 ? (
        ...
        <p className="text-muted-foreground text-sm">{housekeepingMessages.common.none}</p>
      )}
    </div>
  )
}
