...
pms/
  ...
  packages/
    ...
    # admin UI stays app-side (thin-host pattern) in starters/pms/src/components/*,
...
(`starters/pms/src/modules/<name>/` with `defineDeploymentModule`, auto-discovered
...
`starters/pms/src/api/composition.ts` registers them explicitly). Deployment-local
schemas are auto-picked-up by the migration collector, so prototyping is cheap;
graduated package schemas keep their migrations in `starters/pms/migrations/` and are
...
Owned by `starters/pms` (deployment surface, copied from the operator starter and
...
- **Replace `apps/web` and `apps/docs`** (placeholder Next.js apps) with
  `starters/pms`, generated by `voyant new pms` (the CLI clones the operator
...
> resources — see starters/pms/README.md); trips/quotes could not be cleanly
...
`voyant new pms` → `starters/pms`; strip non-stay verticals via `exclude` +
...
- Consumer blueprint: `../acme-travel/apps/admin` (live external consumer of
  published packages; same file shape `starters/pms` will have).
