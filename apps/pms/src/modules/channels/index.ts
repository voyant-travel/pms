 ...
 *
 * Auto-discovered by `modulesFromGlob` in `src/api/composition.ts` under the key
 * `channels`; the module name `pms/channels` mounts admin routes at
 * `/v1/admin/pms/channels/*`. The public webhook mounts at
...
import { channelsAdminRoutes, channelsPublicRoutes } from "./routes.js"

export default defineDeploymentModule({
  module: { name: "pms/channels" },
  adminRoutes: channelsAdminRoutes,
  publicRoutes: channelsPublicRoutes,
...
export { type IngestOutcome, ingestReservation } from "./service-ingest.js"
export {
  findReservation,
  getReservation,
  listReservations,
